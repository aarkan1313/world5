...
| 21 | TERRAIN_RENDERER | draft | [terrain/](../../engine/scripts/terrain/) + [PLAN](../plans/21_TERRAIN_RENDERER_PLAN.md) | Phase 4.4-4.6 + 4.9 shipped 13 modules (incl Phase 4.9.a RingHeightArray) + TerrainWorld composer (590 lines). 280+ tests. Walking demo renders displaced firn-snow alpine with per-fragment slot selection (mid/rock textures active on slopes per catalog selectors). **Audit C1 closed (Phase 4.9.a)**: per-ring RingHeightArray + Texture2DArray binding + shader picks correct page per fragment via world XZ → no more outer-ring chunk seams. **Audit S1 still open**: 3060 perf claim TBD — extrapolation gives 13-17 ms at 4 rings (7-12× over 2.0 ms budget); Phase 5.6 needs real hardware |
