...
| 21 | TERRAIN_RENDERER | draft | [terrain/](../../engine/scripts/terrain/) + [PLAN](../plans/21_TERRAIN_RENDERER_PLAN.md) | Phase 4.4-4.6 shipped: 12 modules across renderer/streaming/material/diagnostics + TerrainWorld composer (430 lines). 250+ tests. Walking demo runs standalone post Phase 4.7+4.8. Frame budget 2.0 ms at high; measured 4-6 ms on RTX 5090 Laptop (F2 trigger engaged for 3060). **Phase 5.5 wire-up**: TerrainWorld._load_world_bundle calls Sibling/DetailTextureArray loaders + binds Layer 1+2 arrays per ring at world load. **Phase 5.4 brown-band fixes**: ClipmapRing sets MeshInstance3D.custom_aabb (Godot 4.6 ignores ArrayMesh override); ClipmapGeometry generates normals + front-facing winding; ring meshes cast_shadow=OFF. Walking demo now renders displaced firn-snow alpine terrain |
